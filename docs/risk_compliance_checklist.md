...
| Authentication | Synthetic prototype: not applicable (no external users, fully local). Live Phase-1 pipeline: username/password authentication is already implemented on the MQTT broker connection (`MQTT_USER`/`MQTT_PASS`, see `phase1/README.md`'s interface contract) -- the dashboard itself has no login, by design, since it's meant to be publicly viewable by judges. Role-separated regulator access (admin vs. read-only) is a real pilot pre-requisite, not yet built. |
| Secrets management | No credentials committed to this repository -- checked directly, not assumed: `git log -p --all` across the full history turns up only the placeholder `CHANGE_ME_NODE_PASSWORD`/`.env.example` templates, never a real MQTT or DB password. `.env.example` (root and `phase1/vps/`) document required variables without values. |
| Encryption | Synthetic prototype: not applicable, fully local, no network traffic. Live Phase-1 pipeline today: MQTT traffic to the VPS is authenticated but **not yet TLS-encrypted in transit** -- an honest, disclosed gap for a bootcamp demo on non-sensitive RF energy readings, not something to claim as solved. Real-pilot data-in-transit (sensing node -> ZCHPC CCE) will use TLS before any production deployment. |
