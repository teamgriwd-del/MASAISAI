...
| Milestones | 0-30 days: sensing nodes deployed at pilot site, dashboard live with real (not simulated) utilization data. 31-60 days: fusion model classifying real readings, real utilization/anomaly dataset accumulating, zero missed-flag incidents against known ground truth. 61-90 days: a real utilization/compliance case reviewed with POTRAZ, scale plan agreed for additional sites -- **not** "anchor institution connected," since that outcome depends on POTRAZ's own separate decision to act on the verified data, not on MASAISAI directly (matches `business_model.md`'s success metrics -- kept identical on purpose so the two docs can't be caught contradicting each other). |
